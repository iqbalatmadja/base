...
@push('cssfiles')
<?php /*<link rel="stylesheet" href="{{ asset('css/styles.css') }}" /> */?>
@endpush

@push('jsfiles')
<?php /*<script src="{{ asset('js/scripts.js') }}"></script>*/?>
@endpush
...
<div class="max-w-[400px] mx-auto w-full">
    <div class="flex flex-col items-center mt-[5vh]">
        ...
        <form action="{{ route('login.process') }}" method="post">
            @csrf
            <input type="email" class="w-full px-6 py-3 mb-2 border border-slate-600 rounded-lg font-medium bg-white @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email') }}" placeholder="Email">
            @if ($errors->has('email'))
            <span class="text-base font-bold text-center text-orange-600">{{ $errors->first('email') }}</span>
            @endif
            <input type="password" class="w-full px-6 py-3 mb-2 border border-slate-600 rounded-lg font-medium bg-white @error('password') is-invalid @enderror" id="password" name="password" placeholder="Password">
